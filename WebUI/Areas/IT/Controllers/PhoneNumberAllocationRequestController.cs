...
{
    [Authorize]
    public class PhoneNumberAllocationRequestController : Controller
    {
        // Идентификатор вида заявки
        private const int SERVICE_ID = 15;
        ...
        private readonly IPhoneNumberAllocationRequestLogic requestLogic;
        private readonly IPhoneNumberAllocationRequestLifeCycleLogic lifeCycleLogic;

        public PhoneNumberAllocationRequestController(IAccountLogic accountLogic, IEmployeeLogic employeeLogic, IAccountPermissionLogic accountPermissionLogic,
           ...
           IPhoneNumberAllocationRequestLogic requestLogic, IPhoneNumberAllocationRequestLifeCycleLogic lifeCycleLogic)
        {
        ...
        /// <returns></returns>
        private async Task PopulateDropDownList(PhoneNumberAllocationRequestViewModel model)
        {
        ...
        private PhoneNumberAllocationRequestLifeCycle InitializeLifeCycle(int requestId, Employee user, string message)
        {
            // создание записи жизненного цикла заявки
            PhoneNumberAllocationRequestLifeCycle lifeCycle = new PhoneNumberAllocationRequestLifeCycle();
        ...
        private bool IsApproval(Service service, List<PhoneNumberAllocationRequestLifeCycle> lifeCycles)
        {
            // флаг согласования
        ...
        [HttpPost]
        public async Task<ActionResult> Create(PhoneNumberAllocationRequestViewModel model)
        {
            // получение данных об авторизованном сотруднике
            // инициализация конфигурации
            var user = await PopulateAccountInfo(model);
        ...
        /// <returns>Возвращает представления согласно логике метода</returns>
        [HttpPost]
        public async Task<ActionResult> Edit(PhoneNumberAllocationRequestViewModel model)
        {
            // получение данных об авторизованном сотруднике
            // инициализация конфигурации
            var user = await PopulateAccountInfo(model);
