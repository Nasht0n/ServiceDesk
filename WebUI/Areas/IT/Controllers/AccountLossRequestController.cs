...
{
    public class AccountLossRequestController : Controller
    {
        private const int SERVICE_ID = 8;
        ...
        private readonly IAccountLossRequestLogic requestLogic;
        private readonly IAccountLossRequestLifeCycleLogic lifeCycleLogic;
        private readonly IAttachmentLogic attachmentLogic;
        private readonly IAccountLossRequestAttachmentLogic requestAttachmentLogic;

        public AccountLossRequestController(IAccountLogic accountLogic, IEmployeeLogic employeeLogic, IAccountPermissionLogic accountPermissionLogic,
            IPriorityLogic priorityLogic, IServiceLogic serviceLogic, ISubdivisionLogic subdivisionLogic, IAccountLossRequestLogic requestLogic,
            IAccountLossRequestLifeCycleLogic lifeCycleLogic,
            IAttachmentLogic attachmentLogic, IAccountLossRequestAttachmentLogic requestAttachmentLogic)
        {
        ...
        private async Task PopulateDropDownList(AccountLossRequestViewModel model)
        {
            var priorities = await priorityLogic.GetPriorities();
            model.Priorities = new SelectList(priorities, "Id", "Fullname");
        }

        private async Task<AccountLossRequest> InitializeRequest(AccountLossRequestViewModel model, Employee user)
        {
            AccountLossRequest request = new AccountLossRequest();
        ...
        private AccountLossRequestLifeCycle InitializeLifeCycle(int requestId, Employee user, string message)
        {
            AccountLossRequestLifeCycle lifeCycle = new AccountLossRequestLifeCycle();
        ...
        public async Task<ActionResult> Details(int id)
        {
            ...
            AccountLossRequest request = await requestLogic.GetRequestById(id);
            var service = await serviceLogic.GetServiceById(request.ServiceId);
            List<AccountLossRequestLifeCycle> lifeCycles = await lifeCycleLogic.GetLifeCycles(request);
            AccountLossDetailsRequestViewModel model = ModelFromData.GetViewModel(request, user, lifeCycles);
            model.AllApproval = IsApproval(service, lifeCycles);
            return View(model);
        }

        private bool IsApproval(Service service, List<AccountLossRequestLifeCycle> lifeCycles)
        {
        ...
        [HttpPost]
        public async Task<ActionResult> Create(AccountLossRequestViewModel model)
        {
            await PopulateDropDownList(model);
            Employee user = await PopulateAccountInfo();
        ...
        public async Task<ActionResult> Edit(int id)
        {
            await PopulateAccountInfo();
            var request = await requestLogic.GetRequestById(id);
            AccountLossRequestViewModel model = ModelFromData.GetViewModel(request);
            ...
            foreach (var attachment in attachments)
            {
                ...
                model.AttachmentsModel.Add(new AccountLossRequestAttachmentViewModel { AttachmentModel = attachmentModel });
            }
            await PopulateDropDownList(model);
            return View(model);
        }
        ...
        [HttpPost]
        public async Task<ActionResult> Edit(AccountLossRequestViewModel model)
        {
            Employee user = await PopulateAccountInfo();
            await PopulateDropDownList(model);
        ...
        [HttpPost]
        public async Task<ActionResult> Delete(int id, AccountLossRequestViewModel model)
        {
            await PopulateAccountInfo();
            var request = await requestLogic.GetRequestById(id);
        ...
        public async Task<ActionResult> AddMessage(int id, AccountLossDetailsRequestViewModel model)
        {
            Employee user = await PopulateAccountInfo();
            var service = await serviceLogic.GetServiceById(SERVICE_ID);
