...
{
    public class AccountRegistrationRequestController : Controller
    {
        private const int SERVICE_ID = 9;
        ...
        private readonly IAccountRegistrationRequestLogic requestLogic;
        private readonly IAccountRegistrationRequestLifeCycleLogic lifeCycleLogic;
        private readonly IAttachmentLogic attachmentLogic;
        private readonly IAccountRegistrationRequestAttachmentLogic requestAttachmentLogic;

        public AccountRegistrationRequestController(IAccountLogic accountLogic, IEmployeeLogic employeeLogic, IAccountPermissionLogic accountPermissionLogic,
            IPriorityLogic priorityLogic, IServiceLogic serviceLogic, ISubdivisionLogic subdivisionLogic, IAccountRegistrationRequestLogic requestLogic,
            IAccountRegistrationRequestLifeCycleLogic lifeCycleLogic,
            IAttachmentLogic attachmentLogic, IAccountRegistrationRequestAttachmentLogic requestAttachmentLogic)
        {
        ...
        private async Task PopulateDropDownList(AccountRegistrationRequestViewModel model)
        {
            var priorities = await priorityLogic.GetPriorities();
            model.Priorities = new SelectList(priorities, "Id", "Fullname");
        }

        private async Task<AccountRegistrationRequest> InitializeRequest(AccountRegistrationRequestViewModel model, Employee user)
        {
        ...
        private AccountRegistrationRequestLifeCycle InitializeLifeCycle(int requestId, Employee user, string message)
        {
            AccountRegistrationRequestLifeCycle lifeCycle = new AccountRegistrationRequestLifeCycle();
        ...
        public async Task<ActionResult> Details(int id)
        {
            ...
            AccountRegistrationRequest request = await requestLogic.GetRequestById(id);
            var service = await serviceLogic.GetServiceById(request.ServiceId);
            List<AccountRegistrationRequestLifeCycle> lifeCycles = await lifeCycleLogic.GetLifeCycles(request);
            AccountRegistrationDetailsRequestViewModel model = ModelFromData.GetViewModel(request, user, lifeCycles);
            model.AllApproval = IsApproval(service, lifeCycles);
            return View(model);
        }

        private bool IsApproval(Service service, List<AccountRegistrationRequestLifeCycle> lifeCycles)
        {
        ...
        [HttpPost]
        public async Task<ActionResult> Create(AccountRegistrationRequestViewModel model)
        {
            await PopulateDropDownList(model);
            Employee user = await PopulateAccountInfo();
            ...
            foreach (var file in model.Files)
            {
                ...
                AccountRegistrationRequestAttachment attachment = new AccountRegistrationRequestAttachment
                {
        ...
        public async Task<ActionResult> Edit(int id)
        {
            await PopulateAccountInfo();
            var request = await requestLogic.GetRequestById(id);
            AccountRegistrationRequestViewModel model = ModelFromData.GetViewModel(request);
            ...
            foreach (var attachment in attachments)
            {
                ...
                model.AttachmentsModel.Add(new AccountRegistrationRequestAttachmentViewModel { AttachmentModel = attachmentModel });
            }
            await PopulateDropDownList(model);
            return View(model);
        }
        ...
        [HttpPost]
        public async Task<ActionResult> Edit(AccountRegistrationRequestViewModel model)
        {
            Employee user = await PopulateAccountInfo();
            await PopulateDropDownList(model);
        ...
        public async Task<ActionResult> AddMessage(int id, AccountRegistrationDetailsRequestViewModel model)
        {
            Employee user = await PopulateAccountInfo();
            var service = await serviceLogic.GetServiceById(SERVICE_ID);
