...
{
    public class SoftwareDevelopmentRequestController : Controller
    {
        private const int SERVICE_ID = 18;
        ...
        private readonly ISoftwareDevelopmentRequestLogic requestLogic;
        private readonly ISoftwareDevelopmentRequestLifeCycleLogic lifeCycleLogic;
        private readonly IAttachmentLogic attachmentLogic;
        private readonly ISoftwareDevelopmentRequestAttachmentLogic requestAttachmentLogic;

        public SoftwareDevelopmentRequestController(IAccountLogic accountLogic, IEmployeeLogic employeeLogic, IAccountPermissionLogic accountPermissionLogic,
            IPriorityLogic priorityLogic, IServiceLogic serviceLogic, ISubdivisionLogic subdivisionLogic, ISoftwareDevelopmentRequestLogic requestLogic,
            ISoftwareDevelopmentRequestLifeCycleLogic lifeCycleLogic,
            IAttachmentLogic attachmentLogic, ISoftwareDevelopmentRequestAttachmentLogic requestAttachmentLogic)
        {
        ...
        private async Task PopulateDropDownList(SoftwareDevelopmentRequestViewModel model)
        {
            var priorities = await priorityLogic.GetPriorities();
            model.Priorities = new SelectList(priorities, "Id", "Fullname");
        }

        private async Task<SoftwareDevelopmentRequest> InitializeRequest(SoftwareDevelopmentRequestViewModel model, Employee user)
        {
            SoftwareDevelopmentRequest request = new SoftwareDevelopmentRequest();
        ...
        private SoftwareDevelopmentRequestLifeCycle InitializeLifeCycle(int requestId, Employee user, string message)
        {
            SoftwareDevelopmentRequestLifeCycle lifeCycle = new SoftwareDevelopmentRequestLifeCycle();
        ...
        public async Task<ActionResult> Details(int id)
        {
            ...
            SoftwareDevelopmentRequest request = await requestLogic.GetRequestById(id);
            var service = await serviceLogic.GetServiceById(request.ServiceId);
            List<SoftwareDevelopmentRequestLifeCycle> lifeCycles = await lifeCycleLogic.GetLifeCycles(request);
            SoftwareDevelopmentDetailsRequestViewModel model = ModelFromData.GetViewModel(request, user, lifeCycles);
            model.AllApproval = IsApproval(service, lifeCycles);
            return View(model);
        }

        private bool IsApproval(Service service, List<SoftwareDevelopmentRequestLifeCycle> lifeCycles)
        {
        ...
        [HttpPost]
        public async Task<ActionResult> Create(SoftwareDevelopmentRequestViewModel model)
        {
            await PopulateDropDownList(model);
            Employee user = await PopulateAccountInfo();
        ...
        public async Task<ActionResult> Edit(int id)
        {
            await PopulateAccountInfo();
            var request = await requestLogic.GetRequestById(id);
            SoftwareDevelopmentRequestViewModel model = ModelFromData.GetViewModel(request);
            ...
            foreach (var attachment in attachments)
            {
                ...
                model.AttachmentsModel.Add(new SoftwareDevelopmentRequestAttachmentViewModel { AttachmentModel = attachmentModel });
            }
            await PopulateDropDownList(model);
            return View(model);
        }
        ...
        [HttpPost]
        public async Task<ActionResult> Edit(SoftwareDevelopmentRequestViewModel model)
        {
            Employee user = await PopulateAccountInfo();
            await PopulateDropDownList(model);
        ...
        [HttpPost]
        public async Task<ActionResult> Delete(int id, SoftwareDevelopmentRequestViewModel model)
        {
            await PopulateAccountInfo();
            var request = await requestLogic.GetRequestById(id);
