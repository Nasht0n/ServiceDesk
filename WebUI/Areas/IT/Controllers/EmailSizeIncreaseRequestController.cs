...
{
    public class EmailSizeIncreaseRequestController : Controller
    {
        private const int SERVICE_ID = 12;
        ...
        private readonly IEmailSizeIncreaseRequestLogic requestLogic;
        private readonly IEmailSizeIncreaseRequestLifeCycleLogic lifeCycleLogic;
        private readonly IEquipmentLogic equipmentLogic;

        public EmailSizeIncreaseRequestController(IAccountLogic accountLogic, IEmployeeLogic employeeLogic, IAccountPermissionLogic accountPermissionLogic,
           ...
           IEmailSizeIncreaseRequestLogic requestLogic, IEmailSizeIncreaseRequestLifeCycleLogic lifeCycleLogic, IEquipmentLogic equipmentLogic)
        {
        ...
        private async Task PopulateDropDownList(EmailSizeIncreaseRequestViewModel model)
        {
            var priorities = await priorityLogic.GetPriorities();
            model.Priorities = new SelectList(priorities, "Id", "Fullname");
        }

        private async Task<EmailSizeIncreaseRequest> InitializeRequest(EmailSizeIncreaseRequestViewModel model, Employee user)
        {
            EmailSizeIncreaseRequest request = new EmailSizeIncreaseRequest();
        ...
        private EmailSizeIncreaseRequestLifeCycle InitializeLifeCycle(int requestId, Employee user, string message)
        {
            EmailSizeIncreaseRequestLifeCycle lifeCycle = new EmailSizeIncreaseRequestLifeCycle();
        ...
        public async Task<ActionResult> Details(int id)
        {
            ...
            EmailSizeIncreaseRequest request = await requestLogic.GetRequestById(id);
            List<EmailSizeIncreaseRequestLifeCycle> lifeCycles = await lifeCycleLogic.GetLifeCycles(request);
            EmailSizeIncreaseDetailsRequestViewModel model = ModelFromData.GetViewModel(request, user, lifeCycles);
            return View(model);
        }
        ...
        [HttpPost]
        public async Task<ActionResult> Create(EmailSizeIncreaseRequestViewModel model)
        {
            await PopulateDropDownList(model);
            Employee user = await PopulateAccountInfo();
        ...
        public async Task<ActionResult> Edit(int id)
        {
            await PopulateAccountInfo();
            var request = await requestLogic.GetRequestById(id);
            EmailSizeIncreaseRequestViewModel model = ModelFromData.GetViewModel(request);
            await PopulateDropDownList(model);
            return View(model);
        }
        [HttpPost]
        public async Task<ActionResult> Edit(EmailSizeIncreaseRequestViewModel model)
        {
            Employee user = await PopulateAccountInfo();
            await PopulateDropDownList(model);
