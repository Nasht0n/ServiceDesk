$("body").on("click", "#btnAddInstallationEquipments", function (val) {
    ...
    var innerEquipmentText = '<input type = "text" class="form-control tblItem bg-transparent border-0" readonly name = "Model.Installations[' + counter + '].EquipmentTypeModel.Name" value = "' + selectedText + '" /> ';
    cell.html(innerEquipmentText);

    cell = $(row.insertCell(-1));
    cell.attr("class", "w-100");
    var innerCount = '<input type = "text" class="form-control tblItem bg-transparent border-0" readonly name = "Model.Installations[' + counter + '].Count" value = "' + count.val() + '" /> ';
