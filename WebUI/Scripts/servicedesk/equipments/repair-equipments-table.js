$("body").on("click", "#btnAddRepairEquipments", function (val) {
    ...
    var innerEquipmentText = '<input type = "text" class="form-control tblItem bg-transparent border-0" readonly name = "Model.Repairs[' + counter + '].ConsumableModel.Name" value = "' + selectedText + '" /> ';
    cell.html(innerEquipmentText);
    cell = $(row.insertCell(-1));
    cell.attr("class", "w-100");
    var innerCount = '<input type = "text" class="form-control tblItem bg-transparent border-0" readonly name = "Model.Repairs[' + counter + '].Count" value = "' + count.val() + '" /> ';
