$("body").on("click", "#btnAddReplaceEquipments", function (val) {
    ...
    var innerEquipmentTypeId = '<input type="hidden" class="readonly" name="Model.Replaces[' + counter + '].EquipmentTypeId" value="' + selectedValue + '"/>';
    cell.attr("class", "d-none");
    cell.html(innerEquipmentTypeId);
    var cell = $(row.insertCell(-1));
    var innerEquipmentTypeModelId = '<input type="hidden" class="readonly" name="Model.Replaces[' + counter + '].EquipmentTypeModel.Id" value="' + selectedValue + '"/>';
    cell.attr("class", "d-none");
