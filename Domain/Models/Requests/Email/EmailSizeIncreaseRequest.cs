...
{
    /// <summary>
    /// Класс-сущность, содержащий описание данных для хранения в таблице "Заявка на увеличение объема почтового ящика"
    /// </summary>
    public class EmailSizeIncreaseRequest:Request
    {
        ...
        /// </summary>
        public EmailSizeIncreaseRequest()
        {
        ...
        public EmailSizeIncreaseRequest(string title, string justification, string description, int serviceId, int statusId, int priorityId, int clientId, int executorGroupId, string email)
        {
            Title = title;
        ...
        /// </summary>
        /// <returns>Возвращает строковое представление объекта заявки на увеличение объема почтового ящика.</returns>
        public override string ToString()
        {
            return $"EmailSizeIncreaseRequest object:(Id:[{Id}];Title:[{Title}];Justification:[{Justification}];Description:[{Description}];ServiceId:[{ServiceId}];" +
                $"StatusId:[{StatusId}];PriorityId:[{PriorityId}];ClientId:[{ClientId}];ExecutorGroupId:[{ExecutorGroupId}]);Email:[{Email}].";
        }
        /// <summary>
        ...
        {
            if (obj is EmailSizeIncreaseRequest && obj != null)
            {
                EmailSizeIncreaseRequest temp = (EmailSizeIncreaseRequest)obj;
