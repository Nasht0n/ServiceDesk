...
namespace Domain.Models.Requests.Accounts
{
    ...
    public class AccountDisconnectRequest:Request
    {
        ...
        public AccountDisconnectRequest(string title, string justification, string description, int serviceId, int statusId, int priorityId, int cabinetId, int clientId, int executorGroupId)
        {
            Title = title;
        ...
        /// </summary>
        /// <returns>Возвращает строковое представление объекта заявки на отключение учетной записи.</returns>
        public override string ToString()
        {
            return $"AccountDisconnectRequest object:(Id:[{Id}];Title:[{Title}];Justification:[{Justification}];Description:[{Description}];ServiceId:[{ServiceId}];" +
        ...
        {
            if (obj is AccountDisconnectRequest && obj != null)
            {
                AccountDisconnectRequest temp = (AccountDisconnectRequest)obj;
