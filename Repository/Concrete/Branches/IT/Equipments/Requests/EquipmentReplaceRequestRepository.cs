...
{
    /// <summary>
    /// Класс доступа к хранилищу заявок на замену оборудования
    /// </summary>
    public class EquipmentReplaceRequestRepository : IEquipmentReplaceRequestRepository
    {
        ...
        {
            log.Debug($"Метод добавления записи заявки на замену оборудования");
            try
            {
        ...
        {
            log.Debug($"Метод получения списка записей заявки на замену оборудования");
            try
            {
                log.Debug($"Начало выполнения метода.");
                // старт таймера
                ...
                var list = await context.EquipmentReplaceRequests
                    ...
                    .Include(a => a.ReplaceEquipments.Select(re => re.EquipmentType))
                    .ToListAsync();
                // остановка таймера
                watch.Stop();
            ...
            {
                log.Error($"Ошибка получения списка записей заявки на замену оборудования: {ex.Message}.");
                return null;
            }
        }
        /// <summary>
        ...
        {
            log.Debug($"Метод редактировния записи заявки на замену оборудования");
            try
            {
                log.Debug($"Начало выполнения метода.");
                // старт таймера
